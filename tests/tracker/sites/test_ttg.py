"""听听歌（TTG / ToTheGlory.im）站点集成测试。

使用方法
--------
1. 用浏览器登录 https://totheglory.im，打开 DevTools →
   Network → 任意请求 → Request Headers → 复制 Cookie 整行的值。
2. 在仓库根目录的 .env 中填写::

       MOVIECLAW_TEST_COOKIES_TTG="uid=...; pass=...; ssl=yes"

   .env 已在 .gitignore 中，凭据不会进 git。
3. 运行测试::

       pytest tests/tracker/sites/test_ttg.py -v -s

注意
----
- 所有 case 依赖真实 cookie，未填写时自动跳过，不影响 CI。
- 测试只读取远端数据，不会下载种子文件。
- TTG 的列表页不支持标准分类参数，分类过滤相关 case 统一走 search()。
"""
...
TEST_COOKIES: str = os.getenv("MOVIECLAW_TEST_COOKIES_TTG", "")

pytestmark = pytest.mark.integration

_NO_COOKIE = not TEST_COOKIES.strip()
_SKIP = pytest.mark.skipif(_NO_COOKIE, reason="未配置 MOVIECLAW_TEST_COOKIES_TTG，跳过集成测试")
...
async def test_list_first_page(site):
    ...
    assert "/t/" in first.detail_url, "TTG 详情链接应为 /t/{id} 路径格式"
    assert first.site_category_id, "TTG 分类 ID（中文 alt 文本）不应为空"

    print(f"\n首页共 {len(page.items)} 条，估计总页数: {page.total_pages}")
    print(f"第 1 条: [{first.site_category_id}] {first.title}")
    print(
        f"  做种={first.seeders}  吸血={first.leechers}  "
        f"大小={first.size}  免费={first.free}"
    )
...
@_SKIP
async def test_search_movie_category(site):
    """电影分类搜索，验证 TTG 的 category:`分类名` 过滤逻辑。"""
    result = await site.search(SearchQuery(categories=[TorrentCategory.MOVIE]))

    assert result.items, "电影分类搜索结果不应为空"
    print(f"\n电影分类返回 {len(result.items)} 条")
    for item in result.items[:5]:
        print(f"  [{item.site_category_id}] {item.title[:60]}")


@_SKIP
async def test_search_tv_category(site):
    """剧集分类搜索，验证 TTG 的中文分类映射。"""
    result = await site.search(SearchQuery(categories=[TorrentCategory.TV]))

    assert result.items, "剧集分类搜索结果不应为空"
    print(f"\n剧集分类返回 {len(result.items)} 条")
    for item in result.items[:5]:
        print(f"  [{item.site_category_id}] {item.title[:60]}")
